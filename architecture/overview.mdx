...
├── container/
│   ├── ApplicationContext         // facade — ачаалж эхлэх (startup)/shutdown
│   ├── ApplicationContextBuilder  // fluent API
│   ├── BeanScanner                // classpath scan
│   ├── BeanRegistry               // bean definition + cache
│   ├── DependencyInjector         // field/байгуулагчийн хамаарал оруулалт (constructor injection)
│   ├── ReflectionCache            // class metadata cache
│   └── visualize/DependencyTreeHtmlExporter
├── Үзүүлэлт/          // BenchmarkRunner, Result, Report
...
sequenceDiagram
    ...
    Ctx->>Reg: хайлт (lookup)(type)
    alt singleton present
        Reg-->>Ctx: cached instance
    else хүлээлттэй (lazy) эсвэл прототип
...
| --- | --- |
| `ApplicationContext` | Facade — ачаалж эхлэх (startup)/shutdown оркестрчиглэл, `getBean(...)`, `close()`. |
...
| `BeanRegistry` | `Map<String, BeanDefinition>`, `Map<String, Object>` (синглтон нөөц (cache)), prototype factory. |
...
| `creationOrder: List<String>` | `@PreDestroy` reverse давталт-нд ашиглана |
...
[Архитектурын шийдвэрүүд](/architecture/design-decisions) хуудаснаас тус бүрийн сонголтын
